    ...
    for iteration in 0..max_iterations {
        ...
        for entry in ordered_results {
            if let Some((tool_name, tool_call_id, outcome)) = entry {
                // Extract media from successful tool results (mobile: photo sending)
                if outcome.success {
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&outcome.output) {
                        if let Some(media) = parsed.get("media") {
                            if media.get("type").and_then(serde_json::Value::as_str) == Some("photo") {
