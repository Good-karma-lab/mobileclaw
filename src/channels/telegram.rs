...
Allowlist Telegram username (without '@') or numeric user ID.",
        ...
        let _ = self
            .send(&SendMessage::new(
                format!(
                    "🔐 This bot requires pairing from MobileClaw app.\n\nSend this exact message in MobileClaw chat:\n`MobileClaw telegram bot pairing: {suggested_identity}`\n\nAfter pairing succeeds, send your message again."
                ),
                &chat_id,
            ))
            .await;
...
impl Channel for TelegramChannel {
    ...
    async fn send_photo(
        ...
        let (chat_id, thread_id) = TelegramChannel::parse_reply_target(recipient);
        TelegramChannel::send_photo(self, &chat_id, thread_id.as_deref(), Path::new(file_path), caption).await
    }
